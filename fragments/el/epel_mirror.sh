...
function config_mkosi_pre::epel_from_mirror() {
	log info "Adding EPEL from dl.fedoraproject.org mirror for EL release ${EL_RELEASE} OS_ARCH: ${OS_ARCH}, TOOLCHAIN_ARCH: ${TOOLCHAIN_ARCH}"
	...
	mkosi_stdin_to_work_file "package-manager-tree/etc/yum.repos.d" "epel-${EL_RELEASE}.repo" <<- EPEL_YUM_REPO_MIRROR
		...
		baseurl=https://dl.fedoraproject.org/pub/epel/${EL_RELEASE}/Everything/${TOOLCHAIN_ARCH}/
		gpgcheck=1
		enabled=1
